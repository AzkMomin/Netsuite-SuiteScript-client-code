...
define(['N/record', 'N/search'], function (record, search) {
    function afterSubmit(context) {
        ...
        if (context.type == context.UserEventType.EDIT || context.type == context.UserEventType.CREATE) {
            ...
            OppLineIds.forEach(function (result) {
            ...
            OppLineIds.forEach(function (oppLineId) {
                // log.debug('oppLineId : ' + oppLineId)
                var sfs_oppLineRecord = record.load({
                    'type': 'customrecord_sc_sfs_opportunity_line',
                    'id': oppLineId
                })
                ...
                if (productId == 416 || productId == 414 || productId == 246 || productId == 419) { //
                    ...
                    for (var j = 0; j < pricingLineItemsCount; j++) {
                        ...
                        if (itemId == productId) {
                            ...
                            var itemDateString = getFormatedDateString(pricingItemstartDate)
                            ...
                            if (itemDateString == letestPricePlanDate) {
                            ...
                            if (subFlag === true) {
                                ...
                                if (itemId == productId) {
                                    ...
                                    if (itemDateString == letestPricePlanDate) {
                                        ...
                                        log.debug('pricePlanData update needed: ', pricePlanData)
    ...
    function getOppLineIds(sfs_oppId) {
        var ids = []
        var customrecord_sc_sfs_opportunity_lineSearchColInternalId = search.createColumn({ name: 'internalid' });
        var customrecord_sc_sfs_opportunity_lineSearchColOpportunityParent = search.createColumn({ name: 'custrecord_sc_sfs_opp_line_par_opp' });
        var customrecord_sc_sfs_opportunity_lineSearchColItemId = search.createColumn({ name: 'custrecord_sc_sfs_opp_line_item_id' });

        var customrecord_sc_sfs_opportunity_lineSearch = search.create({
            type: 'customrecord_sc_sfs_opportunity_line',
            filters: [
        ...
        var customrecord_sc_sfs_opportunity_lineSearchPagedData = customrecord_sc_sfs_opportunity_lineSearch.runPaged({ pageSize: 1000 });
        for (var i = 0; i < customrecord_sc_sfs_opportunity_lineSearchPagedData.pageRanges.length; i++) {
            var customrecord_sc_sfs_opportunity_lineSearchPage = customrecord_sc_sfs_opportunity_lineSearchPagedData.fetch({ index: i });
            customrecord_sc_sfs_opportunity_lineSearchPage.data.forEach(function (result) {
                var internalId = result.getValue(customrecord_sc_sfs_opportunity_lineSearchColInternalId);
                var opportunityParent = result.getValue(customrecord_sc_sfs_opportunity_lineSearchColOpportunityParent);
    ...
    function getletestPricePlanDate(sfs_opp, itemId) {
        ...
        var effectiveStartDateString = getFormatedDateString(effectiveDate);
        ...
        for (var s = 0; s < pricingLineItemsCount; s++) {
            ...
            if (ItemId == itemId) {//ItemId == 416 || ItemId == 414 || ItemId == 246 || ItemId == 419
                var startDate = subscriptionRecord.getSublistValue({
                    'sublistId': 'priceinterval',
                    'fieldId': 'startdate',
                    'line': s,
                })
                var formatedStartDate = getFormatedDateString(startDate);

                // Checking for year then month day day
                if (ItemId == 416) {
                    //Compare effective date is grater then date
                    var checkDate = getCheckDate(effectiveStartDateString, formatedStartDate);

                    if (checkDate.isGrater) {
                        FleetDates.push(formatedStartDate)
                    }
                    ...
                    if (checkDate.isGrater) {
                        ...
                        connectedCarDates.push(formatedStartDate)
                    }
                    ...
                    if (checkDate.isGrater) {
                        tollManagementDates.push(formatedStartDate)
                    }
                    ...
                    if (checkDate.isGrater) {
                        telematicsDates.push(formatedStartDate)
                    }
    ...
    function getCheckDate(effectiveStartDate, currentStartDate) {

        var effDate = new Date(effectiveStartDate);
    ...
    function getFormatedDateString(fdate) {
        var date = fdate;
