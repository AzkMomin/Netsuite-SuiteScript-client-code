...
define([
    ...
    const createCommRecord = (person) => {
        try {
            var commRec;
            var PreviousSalesRepRecord = getPreviousSalesRepRecord(person.internalID);
            log.debug('PreviousPartnerRecord : ', PreviousSalesRepRecord)
            if (PreviousSalesRepRecord.isFound) {
                log.debug("Previous rec found with ID : ", PreviousSalesRepRecord.internalID)
                commRec = record.load({
            ...
            commRec.setValue({ fieldId: 'custrecord_amz_dfs_com_emp', value: person.internalID });
            ...
            var SalesrepAccumulatedComm = getSalesrepAccumulatedComm(person.internalID);
            var accumulatedComm = 0;
            if (SalesrepAccumulatedComm.isFound) {
                accumulatedComm += Number(SalesrepAccumulatedComm.accuComm);
            }
    ...
    const getData = () => {
        ...
        var salesrepIds = getSalesRep()
        // Partners ID whose commission record has created of current month
        var salesrepIDOfCommRec = getSalesRepIDOfCommRec();
      
        for (var i in salesrepIds) {
            salesrepIDOfCommRec.forEach((result) => {
                if (salesrepIDOfCommRec.indexOf(salesrepIds[i].internalID) != -1) {
                    salesrepIds.splice(i, 1)
                }
            })
        }
        // log.debug('salesrepIds : ', salesrepIds)
        // log.debug('salesrepIDOfCommRec : ', salesrepIDOfCommRec)
        return salesrepIds
    }
    const getSalesRep = () => {
        var salesrepIds = []
    ...
    const getSalesRepIDOfCommRec = () => {
        var commRecSalesrepId = []
        var mySearch = search.load({
            id: 3045
        });
        mySearch.run().each(function (result) {
            ...
            var salesrepId = result.getValue({
                name: 'internalid', join: 'CUSTRECORD_AMZ_DFS_COM_EMP'
            });

            commRecSalesrepId.push(salesrepId)
            return true;
        });
        return commRecSalesrepId
    }
    return {
        getInputData,
        map,
