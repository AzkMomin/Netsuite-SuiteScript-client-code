...
define(['N/url', 'N/search', 'N/currentRecord'],
  function (url, search, currentRecord) {
    ...
    function createConsolatedANS() {
      var record = currentRecord.get();
      // log.debug('rec : ' , record);
      // log.debug('id : ', ID);

      var lineCount = record.getLineCount({ sublistId: 'custpage_table' });
      log.debug('lineCount : ', lineCount);
      var urlArr = []
      for (var i = 0; i < lineCount; i++) {

        var apply = record.getSublistValue({
        ...
        if (apply == true) {
          ...
          //   const Promise = await callAsnScript(url) 
      ...
      // function callAsnScript(url){
      //   window.open(url, 'chromeWindow', 'ASN Popup')
      // }
      var loop =1;
      urlArr.forEach((result)=>{
        log.debug('internalId : ', result);
        var win = window.open(result, `chromeWindow${loop}`);
    ...
    return {
      ...
      createConsolatedANS: createConsolatedANS
    }
