...
define(['N/ui/serverWidget', 'N/search', 'N/render', 'N/record'],
  function (serverWidget, search, render, record) {
      ...
      if (context.request.method === 'GET') {
        ...
        for (var i = 0; i < lineCount; i++) {
          ...
          if (apply === 'T') {
            ...
            let sublistValues = getSublistValue(context, i)

            let salesOrderRec = record.load({
              type: 'salesorder',
              id: parseInt(sublistValues.internalId),
              isDynamic: false,
            })

            var binLocation = salesOrderRec.getValue({ fieldId: 'location' })
            log.debug('binLocation  :', binLocation)
            // log.debug('lineUniqueKey  :', sublistValues.lineUniqueKey)
            ...
            //   if (parseInt(sublistValues.lineKey) == parseInt(sublistValues.lineUniqueKey)) {
            //     log.debug('Found  :')
            //     log.debug('lineKey  :', sublistValues.lineKey)
            ...
            // try {
            //For bins and items
            if (data.size == 0) { data.set(i, [sublistValues.item, parseInt(sublistValues.quantity), binLocation]); }
            else {
              var sameItemFound = false;
              data.forEach(function (value, key) {
                if (value[0] == sublistValues.item && sublistValues.binNumber == value[2]) {
                  log.debug('if')
                  data.set(parseInt(key), [sublistValues.item, parseInt(value[1]) + parseInt(sublistValues.quantity), binLocation]); sameItemFound = true;
                }

              })
              if (sameItemFound == false) {
                data.set(i, [sublistValues.item, parseInt(sublistValues.quantity), binLocation]);
              }
            }
            //For customers, sales order and purchase order

            if (customer.size == 0) {
              customer.set(sublistValues.cust, [1, 1]);
              SO_Array.push(sublistValues.salesOrder);
              PO_Array.push(sublistValues.purchaseOrder);
              ...
              customer.forEach(function (value, key) {
                ...
                if (key == sublistValues.cust) {
                  if (SO_Array.includes(sublistValues.salesOrder) == false) { // SO not found
                    if (PO_Array.includes(sublistValues.purchaseOrder) == false) { // PO not found
                      customer.set(sublistValues.cust, [parseInt(value[0]) + 1, parseInt(value[1]) + 1]);
                      isTrue = true;
                      SO_Array.push(sublistValues.salesOrder);
                      PO_Array.push(sublistValues.purchaseOrder);
                    }
                    else {// PO found
                      customer.set(sublistValues.cust, [parseInt(value[0]) + 1, parseInt(value[1])]);
                      isTrue = true;
                      SO_Array.push(sublistValues.salesOrder);
                    }
                  }
                    ...
                    if (PO_Array.includes(sublistValues.purchaseOrder) == false) { // PO not found
                      ...
                      customer.set(sublistValues.cust, [parseInt(value[0]), parseInt(value[1]) + 1]);
                      isTrue = true;
                      PO_Array.push(sublistValues.purchaseOrder);
                    }
                    else { // PO found
                      customer.set(sublistValues.cust, [parseInt(value[0]), parseInt(value[1])]);
                      isTrue = true;
                    }
        ...
        // var xmlStr = createPdf(rowsForBins, rowsForItems, grandTotal, orderCount);
    ...
    function getSublistValue(context, i) {
    ...
    function createPdf(rowsForBins, rowsForItems, grandTotal, orderCount) {
    ...
    function getBins(binLocation, item, quantity) {
      ...
      var qtyRemains;
      //Search Item In Bins
      var binsItem = searchItemInBins(item, binLocation);
      log.debug('binsItem : ', binsItem)

      if (binsItem.itemInSameLocation.length > 0) {
        let isQtySatisfiedFromSameBin = qtySatisfyFromSameLocation(binsItem.itemInSameLocation, quantity, item);
        if (isQtySatisfiedFromSameBin.quantitySatisfied) {
          return isQtySatisfiedFromSameBin.rowsForBins
        } else {
          qtyRemains = isQtySatisfiedFromSameBin.quantityRemains;
          var isQtySatisfiedFromOtherBin = qtySatisfiedFromOtherBin(qtyRemains, binsItem.itemInDiffLocation, item)
          return isQtySatisfiedFromOtherBin.rowsForBins
        }

      }
      else {
        let qtyRemains = quantity;
        var isQtySatisfiedFromOtherBin = qtySatisfiedFromOtherBin(qtyRemains, binsItem.itemInDiffLocation, item)
        return isQtySatisfiedFromOtherBin.rowsForBins
      }
      ...
      // }
      return {
        'rowsForBins': rowsForBins
      }
    }
    function qtySatisfiedFromOtherBin(quantity, itemInDiffLocation, item) {
      let rowsForBins
      var qtyRemains = parseInt(quantity);
      itemInDiffLocation.forEach((result) => {
        if (qtyRemains >= 0 && result.itemCount > 0) {
          if (qtyRemains > result.itemCount) {
            rowsForBins += getRowsForBins(result.binNumber, item, result.itemCount);
            qtyRemains -= result.itemCount;
            log.debug('remaining Quantiy : ' + qtyRemains)
          }
          else if (qtyRemains == result.itemCount) {
            rowsForBins += getRowsForBins(result.binNumber, item, result.itemCount);
            qtyRemains -= result.itemCount
            log.debug('remaining Quantiy : ' + qtyRemains)
          }
          else if (qtyRemains < result.itemCount && qtyRemains > 0) {
            rowsForBins += getRowsForBins(result.binNumber, item, qtyRemains);
            qtyRemains -= result.itemCount
            log.debug('remaining Quantiy : ' + qtyRemains)
          }
        }
        if (qtyRemains > 0) {
          rowsForBins += getRowsForBins('Null', item, qtyRemains);
          log.debug('remaining Quantiy : ' + quantityRemain)
        }
      })
    ...
    function qtySatisfyFromSameLocation(sameLocationBins, quantity, item) {
      let rowsForBins
      var qtyRemains = parseInt(quantity);
      sameLocationBins.forEach((result) => {
        ...
        else if (parseInt(result.itemCount) > 0) {
          log.debug('quantity is less at same location');
          if (qtyRemains > parseInt(result.itemCount) && qtyRemains > 0) {
            rowsForBins += "<tr>\n" +
              "<th colspan=\"2\" align=\"left\" style=\"width: 30px; \">\n<span style=\"font-size:15px;padding: 10px;\">\n " + result.binNumber + "\n</span>\n</th>" +
              "<th colspan=\"2\" align=\"left\" style=\"width: 30px; \">\n<span style=\"font-size:15px; padding: 10px;\">\n " + item + " \n</span>\n</th>" +
      ...
      if (qtyRemains > 0) {
        ...
        return {
          'quantityRemains': qtyRemains,
          'quantitySatisfied': false,
          'rowsForBins': rowsForBins
        }
      } else {
    ...
    function searchItemInBins(item, binLocation) {
      var itemInDiffLocation = [];
      var itemInSameLocation = [];
      ...
      for (var i = 0; i < inventorydetailSearchPagedData.pageRanges.length; i++) {
        ...
        log.debug('search data length for bins contain particular items :: ', inventorydetailSearchPage.data.length)
        if (inventorydetailSearchPage.data.length != 0) {
          ...
          inventorydetailSearchPage.data.forEach(function (result) {
            ...
            if (binLocation == locationId && itemCount < 0) {
              log.debug('same location found at searchlocation  :')
              itemInSameLocation.push({
                'binNumber': binNumber,
                'itemCount': itemCount,
      ...
      return {
        'itemInDiffLocation': itemInDiffLocation,
        'itemInSameLocation': itemInSameLocation

      }
    }
    function getRowsForBins(binNumber, item, quantity) {
      let rowsForBins = "<tr>\n" +
        "<th colspan=\"2\" align=\"left\" style=\"width: 30px; \">\n<span style=\"font-size:15px;padding: 10px;\">\n " + binNumber + "\n</span>\n</th>" +
        "<th colspan=\"2\" align=\"left\" style=\"width: 30px; \">\n<span style=\"font-size:15px; padding: 10px;\">\n " + item + " \n</span>\n</th>" +
        "<th colspan=\"2\" align=\"left\" style=\"width: 30px; \">\n<span style=\"font-size:15px; padding: 10px;\">\n " + quantity + " \n</span>\n</th>" +
